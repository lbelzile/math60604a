...
Another example is in settings where the effect of explanatories is multiplicative, so that the output is of the form
\begin{align*}
\begin{pmatrix} \text{quantity depending only } \\
\text{on the particular unit}
\end{pmatrix} \times
\begin{pmatrix} \text{quantity depending} \\
 \text{on the explanatories used}\end{pmatrix}
\end{align*}
