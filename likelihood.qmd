...
\end{align*} 
where the entries of the information matrix are
...
\end{align*}
To compute the expected information matrix, we need to compute expectation of terms of the form $\mathsf{E}\{(Y/\lambda)^2\}$, $\mathsf{E}\{(Y/\lambda)^\alpha\}$, $\mathsf{E}[(Y/\lambda)^\alpha\ln\{(Y/\lambda)^\alpha\}]$ and $\mathsf{E}[(Y/\lambda)^\alpha\ln^2\{(Y/\lambda)^\alpha\}]$. By definition,
\begin{align*}
\mathsf{E}\left\{Y^\alpha\} = \int_0^\infty x^\alpha \frac{\alpha}{\lambda^\alpha} x^{\alpha-1}\exp\left\{-(x/\lambda)^\alpha\right\} \mathrm{d} x
\end{align*}
and making a change of variable from $S = (Y/\lambda)^\alpha$ gives $S \sim \mathsf{Exp}(1)$. Such integrals are tabulated in @Gradshteyn.Ryzhik:2014, and are equal to $1$, $\Gamma(\alpha/2+1)$, $1-\gamma$ and $\gamma^2-2\gamma + \pi^2/6$, respectively, where $\gamma \approx `r -digamma(1)`$ is the Euler--Mascherroni constant and $\Gamma(\cdot)$ the gamma function.
<!-- \begin{align*} -->
...
\begin{align*}
i_{\lambda, \lambda} & = n \lambda^{-2}\alpha\left\{ (\alpha+1)\Gamma(\alpha/2+1)-1\right\} \\
...
info <- function(scale, shape){ 
  c(shape/(scale^2)*((shape + 1)*gamma(shape/2+1) - 1),
  (1+digamma(1))/scale, 
  (1+digamma(1)^2+2*digamma(1)+pi^2/6)/shape^2)
}
