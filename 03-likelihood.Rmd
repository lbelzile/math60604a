...
set.seed(24)
waiting_times <- rexp(n = 20, rate = 1/4)
explik <- function(lambda, samp = waiting_times, log = TRUE){
  ll <- sapply(lambda, function(l){sum(log(dexp(x = samp, rate = 1/l)))})
  switch(log, ll, exp(ll))
}
# curve(expr = explik(x, samp = waiting_times, log = TRUE), from = 1.8, to = 10)
library(latex2exp)
ggplot() +
  geom_vline(xintercept = mean(waiting_times), lty = 2, col = "grey", alpha = 0.8) +
  stat_function(xlim = c(2,5), n = 101L, fun = explik, args = list(log = TRUE, samp = waiting_times)) +
  ylab("log-likelihood") +
  xlab(TeX("$\\lambda$")) + 
...
For the sample of waiting time in the subway, the maximum likelihood estimate is $\widehat{\theta}=`r mean(waiting_times)`$, the observed information is $j(\widehat{\theta})=i(\widehat{\theta})=`r length(waiting_times)/mean(waiting_times)^2`$ and the standard error of $\widehat{\theta}$ is $j(\widehat{\theta})^{-1/2}=`r sqrt(mean(waiting_times)^2/length(waiting_times))`$.
...
```{r variablesquanti, fig.cap = "Log-likelihood curve; the three likelihood-based tests, namely Wald, likelihood ratio and score tests, use different information about the function.", echo = FALSE}
...
The Wald-based confidence intervals are by construction **symmetric**: they may include implausible values (e.g., negative values for variances). The Wald-based confidence intervals are not parametrization invariant: if we want intervals for a nonlinear continuous function $h(\theta)$, then in general
$\mathsf{CI}_{W}\{h(\theta)\} \neq h\{\mathsf{CI}_{W}(\theta)\}.$
So, if $[0.1, 0.9]$ is a 95\% Wald-based confidence interval for $\widehat{\beta}$, the Wald-based confidence interval for $\exp(\widehat{\beta})$ **is not** $[\exp(0.1), \exp(0.9)]$.
...
\end{align*}
where $\chi^2_1(1-\alpha)$ is the $(1-\alpha)$ quantile of the  $\chi^2_1$ distribution. If $\boldsymbol{\theta}$ is multidimensional, confidence intervals for $\theta_i$ are derived using the profile likelihood. Likelihood ratio-based confidence intervals are **parametrization invariant**, so $\mathsf{CI}_{R}\{h(\theta)\} = h\{\mathsf{CI}_{R}(\theta)\}$. Because the likelihood is zero if a parameter value falls outside the range of possible values for the parameter, the intervals only include plausible values of $\theta$. In general, the intervals are asymmetric and have better coverage properties.
...
par(mar = c(4,4,1,1))
bc <- MASS::boxcox(lm(waiting_times~1), plotit = FALSE, lambda = seq(-0.1, 1, by = 0.01))
plot(x = bc$x, 
y = bc$y - max(bc$y), 
