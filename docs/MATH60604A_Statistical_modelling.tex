...
This model is nonlinear in \(\alpha_0\), \(\alpha_1\). If we opt for the reciprocal link function \(g(x)=1/x\), we get \(\beta_0+\beta_1 x\), where \(\beta_0 = 1/\alpha_0\) and \(\beta_1 = \alpha_1/\alpha_0\). Since the generalized linear model is estimated using maximum likelihood and the latter are invariant to reparametrization, we get easily the estimated coefficients.
\end{example}
...
\end{align*}
In general, \(\widehat{\boldsymbol{\beta}}\) solves the score equation \(U(\widehat{\boldsymbol{\beta}})=\boldsymbol{0}_{p+1}\), so we can use a Newton--Raphson algorithm to derive the maximum likelihood estimator. This amounts to a first order Taylor series approximation of the score \(U(\widehat{\boldsymbol{\beta}})\) around some current value \(\boldsymbol{\beta}^{(0)}\),
\begin{align*}
\boldsymbol{0}_{p+1} = U(\widehat{\boldsymbol{\beta}}) \stackrel{\cdot}{=} U(\beta^{(t)}) - j(\boldsymbol{\beta}^{(t)}) (\widehat{\boldsymbol{\beta}}-\boldsymbol{\beta}^{(t)})
\end{align*}
If the \((p+1)\) matrix \(j(\boldsymbol{\beta}^{(t)})\) is invertible, we can thus device an iterative algorithm: starting from some initial value \(\boldsymbol{\beta}^{(0)}\), iterate until convergence the formula
\begin{align*}
\beta^{(t+1)} = \beta^{(t)} + j^{-1}(\boldsymbol{\beta}^{(t)})U(\boldsymbol{\beta}^{(t)}).
\end{align*}
...
Under regularity conditions, the maximum likelihood estimators of \(\boldsymbol{\beta}\) and \(\phi\) are jointly normally distributed under mild regularity conditions, \(\widehat{\boldsymbol{\beta}} \stackrel{\cdot}{\sim}\mathsf{No}_{p+1}\{\boldsymbol{\beta}, (\mathbf{X}^\top\mathbf{W}\mathbf{X})^{-1}\}\) where \(\mathbf{W} = \mathrm{diag}\{(\partial \mu_i/\partial \eta_i)^2/\mathsf{Va}(Y_i)\}\) is a diagonal matrix with elements \(w_i=\{g'(\mu_i)^2a_i(\phi)V(\mu_i)\}^{-1}\).
Since the means \(\mu_i\) are unknown, they are replaced by their estimates estimated at \(\widehat{\boldsymbol{\beta}}\).
...
In large samples, both Pearson \(X^2/\phi\) and the deviance \(D/\phi\) follow approximately a \(\chi^2_{n-p-1}\) distribution if the model is correct and \(\phi\) is known (but this result doesn't hold for binary data or binomial data unless \(m\) is very large and \(\boldsymbol{\beta}\) are small).
