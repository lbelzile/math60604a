...
These notes by Léo Belzile (HEC Montréal) are licensed under a \href{http://creativecommons.org/licenses/by-nc-sa/4.0/}{Creative Commons Attribution-NonCommercial-ShareAlike 4.0 International License} and were last compiled on 2020-09-10.
...
\end{align*}
Solving for \(\theta\) gives \(\widehat{\theta} = \overline{y}\), so the maximum likelihood estimator is the sample mean \(\overline{Y}\). The observed information is \(j(\theta) = -n\theta^{-2} + 2\theta^{-3}n\overline{y}\) and \(i(\theta)=\mathsf{E}\{j(\theta)\}=n\theta^{-2}\).
\end{example}
...
For the sample of waiting time in the subway, the maximum likelihood estimate is \(\widehat{\theta}=3.058\), the observed information is \(j(\widehat{\theta})=i(\widehat{\theta})=2.139\) and the standard error of \(\widehat{\theta}\) is \(j(\widehat{\theta})^{-1/2}=0.684\).
...
\caption{Log-likelihood curve; the three likelihood-based tests, namely Wald, likelihood ratio and score tests, use different information about the function.}\label{fig:variablesquanti}
\end{figure}
...
The Wald-based confidence intervals are by construction \textbf{symmetric}: they may include implausible values (e.g., negative values for variances). The Wald-based confidence intervals are not parametrization invariant: if we want intervals for a nonlinear continuous function \(h(\theta)\), then in general
\(\mathsf{CI}_{W}\{h(\theta)\} \neq h\{\mathsf{CI}_{W}(\theta)\}.\)
So, if \([0.1, 0.9]\) is a 95\% Wald-based confidence interval for \(\widehat{\beta}\), the Wald-based confidence interval for \(\exp(\widehat{\beta})\) \textbf{is not} \([\exp(0.1), \exp(0.9)]\).
...
\end{align*}
where \(\chi^2_1(1-\alpha)\) is the \((1-\alpha)\) quantile of the \(\chi^2_1\) distribution. If \(\boldsymbol{\theta}\) is multidimensional, confidence intervals for \(\theta_i\) are derived using the profile likelihood. Likelihood ratio-based confidence intervals are \textbf{parametrization invariant}, so \(\mathsf{CI}_{R}\{h(\theta)\} = h\{\mathsf{CI}_{R}(\theta)\}\). Because the likelihood is zero if a parameter value falls outside the range of possible values for the parameter, the intervals only include plausible values of \(\theta\). In general, the intervals are asymmetric and have better coverage properties.
...
The Wald statistic and the likelihood ratio test for testing individual coefficients of the mean model \(\beta_j\) in a linear regression are equivalent: they give the same \(p\)-value.
...
\hypertarget{likelihood-based-tools-for-model-comparison}{%
\section{Likelihood-based tools for model comparison}\label{likelihood-based-tools-for-model-comparison}}
