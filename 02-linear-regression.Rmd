...
#           show.loglik = TRUE,
#           string.se = "std. error",
#           string.stat = "Wald stat.",
#           string.p = "p-value",
#           string.est = "estimates")  
  # col.order = c("est", "se", "stat", "p","ci")
...
g1 <- ggplot(data = insurance, aes(x = bmi, y = charges, col = smoker)) +
  geom_point() +
...
g2 <- ggplot(data = insurance, aes(x = age, y = charges, col = smobese)) +
  ...
  ylab("charges (in USD)") + 
    theme(legend.position = "bottom") + 
  labs(x = "age", y = "charges (in USD)", col = "smoking/obesity status")
g1 + g2
```
...
<!-- ```{r } -->
<!-- lm_interaction <- lm(charges ~ age*obese*smoker +bmi*obese:smoker, data = insurance) -->
<!-- insurance$res <- resid(lm_interaction) -->
<!-- ggplot(data = insurance, aes(x = bmi, y = res, col = region)) + -->
<!--   geom_point() + -->
<!--   geom_vline(xintercept = 30) +  -->
<!--   xlab("body mass index") + -->
