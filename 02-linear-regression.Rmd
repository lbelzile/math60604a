...
Inference for the $F$-tests, which are described in more mathematical detail in the [Appendix](ftestslm), is equivalent to the likelihood ratio test for the linear model. The software reports the $p$-values calculated using an $F$ distribution, which is the exact null distribution under the assumption of normally distributed errors with constant variance. The chi-square distribution and the $F$-distribution are equivalent for $n$ large and similarly the Student-_t_ distribution and the normal distribution are indistinguishable for large degrees of freedom, except far in the tail.
...
# library(gtsummary)
# tbl_regression(college_lm) %>% bold_labels() %>% add_global_p()
library(pixiedust)
dust(college_lm) %>% 
...
# modelsummary::modelplot(models = list(full = college_lm,
#                                       reduced = update(college_lm, ~. - rank)), coef_omit = 'Interc') +
#     labs(x = 'Coefficients',
#          y = 'Term names',
#          title = 'Linear regression models for college data',
#          caption = "Model with and without `service`")
```
