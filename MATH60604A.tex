...
\begin{tcolorbox}[enhanced jigsaw, arc=.35mm, coltitle=black, opacitybacktitle=0.6, colbacktitle=quarto-callout-important-color!10!white, colframe=quarto-callout-important-color-frame, titlerule=0mm, bottomtitle=1mm, leftrule=.75mm, title=\textcolor{quarto-callout-important-color}{\faExclamation}\hspace{0.5em}{Important}, left=2mm, toptitle=1mm, rightrule=.15mm, colback=white, bottomrule=.15mm, toprule=.15mm, breakable, opacityback=0]
...
\begin{tcolorbox}[enhanced jigsaw, arc=.35mm, coltitle=black, opacitybacktitle=0.6, colbacktitle=quarto-callout-caution-color!10!white, colframe=quarto-callout-caution-color-frame, titlerule=0mm, bottomtitle=1mm, leftrule=.75mm, title=\textcolor{quarto-callout-caution-color}{\faFire}\hspace{0.5em}{Caution}, left=2mm, toptitle=1mm, rightrule=.15mm, colback=white, bottomrule=.15mm, toprule=.15mm, breakable, opacityback=0]
...
versatile workshorse for statistical inference. Linear regression is
used primarily to evaluate the effects of explanatories (oftentimes
treatment in an experimental setting) on the mean response of the
response, or for prediction. It combines a formulation for the mean of a
\textbf{response variable} \(Y_i\) of a random sample of size \(n\) as a
\textbf{linear function} of observed \textbf{explanatories} (also called
predictors or covariates) \(X_1, \ldots, X_p\), \begin{align}
...
\(\beta_0, \ldots, \beta_p\), and \(\beta_0\) is the \textbf{intercept}.
...
mean-zero distribution with constant variance \(\sigma^2\). The variance
term \(\sigma^2\) is included to take into account the fact that no
exact linear relationship links \(\boldsymbol{X}_i\) and \(Y_i\), or
that measurements of \(Y_i\) are subject to error.

In the Gaussian linear model, responses follow a normal distribution and
...
\(\mu + \sigma Z\) for \(Z \sim \mathsf{normal}(0, 1)\). We may thus
rewrite the linear model in terms of the mean plus an error term,
\begin{align*}
...
\end{align*} where \(\varepsilon_i \sim \mathsf{normal}(0,\sigma^2)\) is
the error term specific to observation \(i\), and we assume that the
errors \(\varepsilon_1, \ldots, \varepsilon_n\) are independent and
identically distributed. We fix the expectation or theoretical mean of
...
The function \texttt{lm} is the workshorse for fitting linear models in
\textbf{R}. It takes as input a formula: suppose you have a data frame
containing columns \texttt{x1} and \texttt{x2} (explanatories) and
\texttt{y} (response); you can then call
\texttt{lm(y\ \textasciitilde{}\ x1\ +\ x2)} to fit the linear model
with conditional mean \(\beta_0 + \beta_1 x_1 + \beta_2x_2\). The
explanatory variable \texttt{y} is on the left hand side of the
equation, while the right hand side should contain the predictors,
separated by a \texttt{+} sign if there are more than one. If you
provide the data frame name using \texttt{data}, then the shorthand
\texttt{y\ \textasciitilde{}\ .} (tilde followed by a period) fits a
model including all of the columns of the data frame (but excluding
\texttt{y}) as explanatories.

The type of effect included by the software depends on the class of the
variable. Any numeric or integer variable will be treated as continuous,
including dummies with 0/1 entries. To include categorical variables,
these should be cast to factors (both for strings and for variables that
are encoded using integers (sex, revenue class, level of education,
marital status, etc.)
...
{\centering \includegraphics[width=0.85\textwidth,height=\textheight]{linearmodels_files/figure-pdf/edacollege-1.pdf}
...
We can assign meanings to the mean parameters \(\boldsymbol{\beta}\) in
the linear model by considering what is
...
line for the simple linear regression for horsepower is added.
...
A linear model that only contains a binary variable \(\mathrm{X}\) as
regressor amounts to specifying a different mean for each of two groups:
...
and the fit of response \(y_i\) against fitted values
\(\\widehat{y}_i\). The ordinary least squares line minimizes the
average squared length of the ordinary residuals.}
...
\(\mathbf{X}^\top\boldsymbol{e}=\boldsymbol{0}_p\). The ordinary
...
will use this property to build graphical diagnostics. , and is also
used to construct visual diagnostics of goodness-of-fit.
...
independent. Provided \(\mathbf{X}^\top\mathbf{X}\) is invertible, the
large-sample variance of the ordinary least squares estimator is
...
the model matrix \(\mathbf{X}\), respectively.

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{data}\NormalTok{(college, }\AttributeTok{package =} \StringTok{"hecstatmod"}\NormalTok{) }\CommentTok{\#load data}
\FunctionTok{str}\NormalTok{(college) }\CommentTok{\# Check that categorical variables are factors}
\CommentTok{\# Fit the linear regression}
\NormalTok{linmod }\OtherTok{\textless{}{-}} \FunctionTok{lm}\NormalTok{(salary }\SpecialCharTok{\textasciitilde{}}\NormalTok{ sex }\SpecialCharTok{+}\NormalTok{ rank }\SpecialCharTok{+}\NormalTok{ service }\SpecialCharTok{+}\NormalTok{ field, }
...
It is sometimes possible to linearize a model: consider for example the
Cobb--Douglas production function (\citeproc{ref-Douglas:1976}{Douglas
1976}), which specifies that economic output \(Y\) is related to labour
\(L\) and capital \(C\) via
...
\(e_i = y_i - \widehat{\beta}_0 - \sum_{j=1}^p \widehat{\beta}_j\mathrm{X}_j\).
...
\begin{proposition}[Log linear
models]\protect\hypertarget{prp-linearity}{}\label{prp-linearity}
