...
This chapter gives an introduction to generalized linear models and focuses in particular on logistic regpassion and Poisson regpassion, but only for the case of independent observations.  Extensions of generalized linear models for correlated and longitudinal data, the so-called *generalized linear mixed models* (GLMM), are covered in MATH80621.


## Basic principles 

The starting point is the same as for linear regpassion: we have a random sample of independent observations $(\boldsymbol{Y}, \mathbf{X})$, where $Y$ is the response variable and $\mathrm{X}_1, \ldots, \mathrm{X}_p$ are $p$ explanatory variables or covariates which are assumed fixed (non-random). The goal is to model the mean of the response variable as a function of the explanatory variables.
...
The need for a link function often arises from parameter constraints: for example, the mean $\mu=\pi$ of a Bernoulli distribution is the proportion of successes and  must lie in the interval $(0, 1)$. Similarly, the mean $\mu$ of the Poisson distribution must be positive. For the normal distribution (ordinary linear regpassion model), the mean is unconstrained and the canonical link function is the identity, $\mu_i = \eta_i$.
...
```{example, label="inverslinklm", name = "Transforming a nonlinear regpassion model into a linear model through a link function"}
...
The standardized deviance residuals $\{r_{D_i}\}$ and standardized Pearson residuals $\{r_{P_i}\}$ should have an approximate standard normal distribution in large samples, but their distribution can be skewed. Like in the linear regpassion, we will work with the **jackknifed deviance residuals** for residual plots
\begin{align*}
...
For ordinary linear regpassion, both $r_{D_i}$ and $r_{P_i}$ reduce to the standardized residuals $t_i=e_i\{S^2(1-h_{ii})\}^{-1/2}$.

<!-- Residuals are employed in regpassion models with Poisson, Gamma, and binomial response for $m$ large.  -->
...
\end{align*}
where $\widehat{\boldsymbol{\beta}}_{-j}$ is the estimate obtained by dropping the $j$th observation from the sample. This requires fitting $n$ different models, which is computationally prohibitive. In the linear regpassion, we can calculate the Cook distance from the formula $C_j = (p+1)^{-1}t_i^2h_{ii}/(1-h_{ii})$, where $t_i$ are the standardized residuals defined in the previous section. For generalized linear models, no such expression exists, although a good approximation is $C_j \approx (p+1)^{-1}r_{P_i}^2h_{ii}/(1-h_{ii})$.
...
On the odds scale, the logistic regpassion model is multiplicative: for an increase of $\mathrm{X}_j$ of one unit, *ceteris paribus*, the odds are multiplied by $\exp(\beta_j)$. 
...
```{r diagplotbirthwgt, eval = FALSE, echo = FALSE}
# uniform residuals for logit2
pi <- predict(logit2,type='response')
ui <- as.numeric(as.character(dat$low))*runif(189,0,pi)+(1-as.numeric(as.character(dat$low)))*runif(189,pi,1)
...
par(mfrow=c(2,2))
plot(x=dat$lwt,ui,xlab="mother's weight",ylab='uniform residuals')  
plot(x=predict(logit2,type='link'),ui,xlab='log(p/(1-p))',ylab='uniform residuals')  
hist(ui,xlab='uniform residuals',ylab= "frequency")  
qqplot(seq(from=1/200,by=1/200,to=189/200),ui,cex=0.5,xlab='theoretical quantiles',ylab='uniform residuals'); abline(0,1)
points(seq(from=1/200,by=1/200,to=189/200),e$overall[1,],pch='-',col='grey')
points(seq(from=1/200,by=1/200,to=189/200),e$overall[2,],pch='-',col='grey')
dev.off()
```
...
For binary data, most model checking tools break down due to the nature of the observations. The deviance and Pearson provide no information about the quality of the model adjustment and their null distribution depends on $\widehat{\boldsymbol{\beta}}$. Since observations are 0/1, residuals plots are also useless. One oddity of binary regpassion is that most of the information in the sample is provided by response whose variance is largest (points that have $\pi$ far from 0 or 1). Thus, if one wants to estimate a coefficient $\beta_j$ of large magnitude, the ratio $|\beta_j|/\mathsf{sd}(\beta_j) \to 0$ as $\beta_j \to \infty$: the power for testing that $\beta_j$ tends to zero using a Wald statistic tends to zero.
This can be seen by looking at how the probability $\pi$ changes as a function of changes in the $j$th explanatory $\mathrm{X}_j$: if $\pi = \mathrm{expit}(\beta_0 + \beta_1\mathrm{X}_1 + \cdots + \beta_p\mathrm{X}_p)$, then $\partial \pi/\partial \mathrm{X}_j = \beta_j \pi(1-\pi)$, so the effect of $\beta_j$ is large when $\pi$ is near $0.5$ and near zero if the probabilities are close to the endpoints $0$ or $1$.
