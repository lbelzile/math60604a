...
In the model with buying \texttt{intention} as a function of \texttt{sex} and \texttt{fixation} time, we would \textbf{not} remove the main effect of \texttt{fixation} while keeping the interaction term \texttt{fixation*sex}, even if we fail to reject $\mathscr{H}_0:\beta_2=0$. Without it, the model would become
\begin{align*}
&\mathsf{E}(\texttt{intention} \mid \texttt{fixation}, \texttt{sex}) \\\quad&=
\begin{cases}
